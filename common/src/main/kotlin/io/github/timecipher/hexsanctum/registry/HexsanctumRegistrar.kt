package io.github.timecipher.hexsanctum.registry
...
typealias RegistrarEntry<T> = HexsanctumRegistrar<T>.Entry<out T>

abstract class HexsanctumRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hexsanctum.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexsanctumRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
