...
public class ForgeMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName.equals("io.github.timecipher.hexsanctum.forge.mixin.MixinDatagenMain")) {
            var shouldApply = System.getProperty("hexsanctum.apply-datagen-mixin", "false").equals("true");
            if (shouldApply) {
                Hexsanctum.LOGGER.warn("Applying scuffed datagen mixin. This should not happen if not running datagen!");
            }
            return shouldApply;
        }
        return true;
